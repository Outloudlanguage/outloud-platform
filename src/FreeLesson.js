...
// =========================================
const MobileFreeLesson = ({ onReturnHome, onReturnToRegister }) => {
  ...
  const [metrics, setMetrics] = useState({
    ex1Correct: 0, ex4Correct: 0, ex5Correct: 0, audioRetries: 0, missedOriginalBeforeCompare: false
  });
  ...
  // ---> CUSTOM MOBILE TOUCH DRAG & DROP STATE <---
  ...
  const handleTouchEnd = (e) => {
    ...
    const targetElement = dropTarget ? dropTarget.closest('[data-drop-target]') : null;

    if (targetElement) {
      const targetId = targetElement.getAttribute('data-drop-target');
      processMove(draggingItem.item, draggingItem.source, targetId);
    }
    ...
    <div className="relative min-h-screen w-full font-sans bg-[#eef5fc] overflow-y-auto overflow-x-hidden flex flex-col items-center">
      ...
      {step === 'exercise1' && (
        <div className="relative z-10 w-full px-5 flex flex-col flex-grow animate-fade-in pb-8">
          ...
          <div className="grid grid-cols-2 gap-3 w-full shrink-0">
            {[
              ...
              <div key={col.id} className="flex flex-col items-center w-full space-y-2">
                <div className="w-full aspect-[4/5] rounded-2xl border-[3px] border-[#08203e] overflow-hidden shadow-sm bg-white relative">
                  <img src={col.img} alt={`Location ${col.id}`} className="absolute inset-0 w-full h-full object-cover" />
                </div>
                {/* Touch Drop Target for Slots */}
                <div data-drop-target={col.id} className={`w-[90%] h-10 rounded-full flex items-center justify-center transition-all duration-300 overflow-hidden ${dragSlots[col.id] ? 'bg-transparent border-none' : 'bg-[#eef5fc]/50 border-2 border-dashed border-[#08203e]'}`}>
                  {dragSlots[col.id] && (
                    <div
                      onTouchStart={(e) => handleTouchStart(e, dragSlots[col.id], col.id)}
                      onTouchMove={handleTouchMove}
                      onTouchEnd={handleTouchEnd}
                      style={{ touchAction: 'none' }}
                      className={`w-full h-full bg-[#08203e] text-white rounded-full flex items-center justify-center text-[10px] font-bold font-montserrat text-center px-1 shadow-sm whitespace-pre-line leading-tight ${draggingItem?.item === dragSlots[col.id] ? 'opacity-0' : 'opacity-100'}`}
                    >
                      {dragSlots[col.id]}
                    </div>
                  )}
          ...
          <div className="shrink-0 flex flex-col items-center w-full mt-6 relative">
            {/* Touch Drop Target for Bank */}
            <div data-drop-target="bank" className={`w-full bg-white rounded-2xl p-4 shadow-lg grid grid-cols-2 gap-2 ${isEx1Complete ? 'hidden' : ''}`}>
              {allOptions.map((opt) => {
                if (!availableOptions.includes(opt)) return <div key={`empty-${opt}`} className="w-full h-10"></div>;
                return (
                  <div
                    key={opt}
                    onTouchStart={(e) => handleTouchStart(e, opt, 'bank')}
                    onTouchMove={handleTouchMove}
                    onTouchEnd={handleTouchEnd}
                    style={{ touchAction: 'none' }}
                    className={`w-full h-10 rounded-full font-bold font-montserrat text-[10px] text-center shadow-md flex items-center justify-center whitespace-pre-line leading-tight px-1 transition-colors bg-[#08203e] text-white ${draggingItem?.item === opt ? 'opacity-0' : 'opacity-100'}`}
                  >
                    {opt}
                  </div>
                );
            ...
            {draggingItem && (
              <div
                className="fixed pointer-events-none z-[100] w-[40%] max-w-[150px] h-10 rounded-full font-bold font-montserrat text-[10px] text-center shadow-2xl flex items-center justify-center whitespace-pre-line leading-tight px-2 bg-student-yellow text-outloud-blue border-2 border-outloud-blue scale-105"
                style={{
                  left: dragPos.x,
                  top: dragPos.y,
          ...
          <div className="flex flex-col items-center justify-center mt-6 h-12">
            ...
            {step === 'exercise3' && hasCompared && (
              <div className="flex flex-row items-center justify-center space-x-3 w-full animate-fade-in-up">
                <button onClick={handleRetryEx2} title="Retry Exercise" className="w-12 h-12 flex items-center justify-center bg-white border-2 border-outloud-blue text-outloud-blue rounded-full shadow-md active:scale-95 shrink-0">
      ...
      {step === 'exercises4_and_5' && (
        <div className="relative z-10 w-full px-5 flex flex-col flex-grow animate-fade-in pb-12 pt-4">
          ...
          <div className="w-full bg-white rounded-[2rem] p-5 shadow-lg">
          ...
          {/* MOVED THE BUTTON HIGHER BY REMOVING mt-auto AND USING mt-8 */}
...
// =========================================
// 2. DESKTOP FREE LESSON UI (100% UNTOUCHED)
// =========================================
const DesktopFreeLesson = ({ onReturnHome, onReturnToRegister }) => {
  // Navigation & Video States
  const [step, setStep] = useState('welcome');
  const [replayCount, setReplayCount] = useState(0);
  const [isEnded, setIsEnded] = useState(false);
...
const FreeLesson = (props) => {
  ...
  return isMobile ? <MobileFreeLesson {...props} /> : <DesktopFreeLesson {...props} />;
};
