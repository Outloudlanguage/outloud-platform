...
const MobileFreeLesson = (props) => {
  const {
    step, replayCount, isEnded, metrics, dragSlots, selectedPill, availableOptions,
    isRecording, hasRecorded, hasCompared, activeAudio, audioProgress,
    act4Input1, setAct4Input1, act4Input2, setAct4Input2, act4Touched1, setAct4Touched1,
    act4Touched2, setAct4Touched2, act5Selection, setAct5Selection,
    handleStart, handleContinueToVideo, handleContinueToEx1, getIframeUrl,
    handleDragStart, handleDropOnSlot, handleDropOnBank, handlePillClick, handleSlotClick,
    handleCheckExercise1, isEx1Complete, playOriginalAudio, playRecordedAudio,
    toggleRecording, handleContinueToEx3, handleRetryEx2, handleContinueToEx4,
    isEx45Complete, handleCheckExercises45, listeningScore, comprehensionScore,
    grammarScore, readingScore, speakingScore, overallScore, allOptions,
    onReturnHome, onReturnToRegister
  } = props;

  const MobileProgressBar = ({ label, percent }) => (
    <div className="flex items-center w-full justify-between mb-3">
      <span className="text-outloud-blue font-montserrat text-sm mr-3 font-medium flex-shrink-0 w-32">{label}</span>
      <div className="w-full h-2.5 bg-[#e6f0f9] rounded-full overflow-hidden border border-[#08203e]/10 shadow-inner flex-grow">
  ...
  return (
    <div className="relative min-h-screen w-full font-sans bg-[#eef5fc] overflow-y-auto overflow-x-hidden flex flex-col items-center">
      {/* Background Bubbles */}
      ...
      <div className="relative z-50 w-full flex justify-between items-center px-5 py-4 shrink-0">
        ...
        <button onClick={onReturnToRegister || onReturnHome} className="flex items-center space-x-1 text-outloud-blue font-bold font-montserrat active:text-blue-900 transition">
      ...
      {/* MOBILE UNIT TITLE */}
      {['popup', 'video', 'exercise1', 'exercise2', 'exercise3', 'exercises4_and_5'].includes(step) && (
        <div className="relative z-10 w-full text-center px-4 mb-4 shrink-0">
          <h1 className="text-[22px] font-black text-outloud-blue font-montserrat uppercase tracking-wide leading-tight">
            A1: UNIT 1<br/>LESSON 1
          </h1>
        </div>
      )}
      ...
      {/* STEP 1: WELCOME */}
      {step === 'welcome' && (
        <div className="relative z-10 flex flex-col items-center justify-center flex-grow w-full px-5 animate-fade-in-up pb-10">
          <div className="bg-white/95 backdrop-blur-md rounded-[2rem] shadow-[0_15px_30px_rgba(0,0,0,0.1)] p-8 text-center w-full max-w-sm">
            <h1 className="text-xl font-black text-outloud-blue font-montserrat mb-4 tracking-wide uppercase">¡Bienvenido a tu clase de prueba!</h1>
            <p className="text-sm text-outloud-blue/80 font-montserrat leading-relaxed text-justify mb-8">Prepárate para experimentar nuestro método inmersivo. Olvídate de la teoría rígida y de traducir en tu cabeza. Estás a un paso de comenzar a pensar en inglés de forma natural y fluida.</p>
            <button onClick={handleStart} className="w-full bg-student-yellow text-outloud-blue text-lg font-black font-montserrat py-3.5 rounded-full shadow-md active:scale-95 transition-transform uppercase tracking-widest">START</button>
          </div>
        </div>
      )}
      ...
      {(step === 'popup' || step === 'video') && (
        <div className="relative z-10 w-full flex-grow flex flex-col items-center justify-center px-5 pb-8 min-h-0">
          <div className="w-full max-w-sm aspect-[4/5] bg-[#2a3036] rounded-[2rem] border-4 border-[#3b434b] shadow-xl relative overflow-hidden flex flex-col p-3">
            <div className="relative w-full flex-grow rounded-xl overflow-hidden bg-black">
            ...
            {step === 'popup' && (
              <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-sm flex items-center justify-center p-5">
                <div className="bg-white/95 rounded-[1.5rem] p-6 text-center w-full shadow-2xl animate-fade-in-up">
                  <svg className="w-8 h-8 text-outloud-blue mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                  <p className="text-[11px] text-outloud-blue font-montserrat leading-relaxed mb-5">Vea el video y preste cuidadosa atención al contenido, lo que aprenda en él le servirá más tarde para su <strong className="font-black text-[13px]">LECCIÓN EN VIVO</strong> con un instructor y para sus actividades interactivas.</p>
                  <button onClick={handleContinueToVideo} className="w-full bg-outloud-blue text-white font-black font-montserrat py-3 rounded-xl active:scale-95 transition-transform text-xs uppercase tracking-wide">CONTINUAR</button>
                </div>
              </div>
            )}
          </div>
          ...
          {step === 'video' && (
             <div className="mt-6">
                <button onClick={handleContinueToEx1} className={`w-14 h-14 rounded-full bg-outloud-blue flex items-center justify-center text-white shadow-xl active:scale-95 transition-all ${isEnded ? 'animate-pulse ring-4 ring-student-yellow ring-offset-2 ring-offset-[#eef5fc]' : ''}`}>
                  <svg className="w-8 h-8 ml-1" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                </button>
             </div>
          )}
        </div>
      ...
      {step === 'exercise1' && (
        <div className="relative z-10 w-full px-5 flex flex-col flex-grow animate-fade-in pb-8">
          <div className="mb-4 text-center shrink-0">
            <h2 className="text-[13px] text-outloud-blue font-montserrat uppercase font-black">LESSON 1: ACTIVITY 1 - <span className="font-bold normal-case">Comprehension exercise.</span></h2>
            <p className="text-[11px] font-bold text-outloud-blue font-montserrat uppercase mt-1">DRAG AND DROP: <span className="font-normal normal-case">Match the text to the location.</span></p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 w-full shrink-0">
            {[
              ...
              <div key={col.id} className="flex flex-col items-center w-full space-y-2">
                <div className="w-full aspect-square rounded-2xl border-[3px] border-[#08203e] overflow-hidden shadow-sm bg-white relative">
                  <img src={col.img} alt={`Location ${col.id}`} className="absolute inset-0 w-full h-full object-cover" />
                </div>
                <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDropOnSlot(e, col.id)} onClick={() => handleSlotClick(col.id)} className={`w-[90%] h-8 rounded-full flex items-center justify-center transition-all duration-300 overflow-hidden ${dragSlots[col.id] ? 'bg-transparent border-none' : 'bg-[#eef5fc]/50 border-2 border-dashed border-[#08203e]'}`}>
                  {dragSlots[col.id] && (
                    <div draggable onDragStart={(e) => handleDragStart(e, dragSlots[col.id], col.id)} className="w-full h-full bg-[#08203e] text-white rounded-full flex items-center justify-center text-[9px] font-bold font-montserrat text-center px-1 shadow-sm whitespace-pre-line leading-tight">
                      {dragSlots[col.id]}
                    </div>
                  )}
                </div>
          ...
          <div className="shrink-0 flex flex-col items-center w-full mt-6">
            <div onDragOver={(e) => e.preventDefault()} onDrop={handleDropOnBank} className={`w-full bg-white rounded-2xl p-4 shadow-lg grid grid-cols-2 gap-2 ${isEx1Complete ? 'hidden' : ''}`}>
              {allOptions.map((opt) => {
                if (!availableOptions.includes(opt)) return <div key={`empty-${opt}`} className="w-full h-10"></div>;
                return (
                  <div key={opt} draggable onClick={() => handlePillClick(opt, 'bank')} onDragStart={(e) => handleDragStart(e, opt, 'bank')} className={`w-full h-10 rounded-full font-bold font-montserrat text-[9px] text-center shadow-md flex items-center justify-center whitespace-pre-line leading-tight px-1 transition-colors ${selectedPill === opt ? 'bg-student-yellow text-outloud-blue' : 'bg-[#08203e] text-white'}`}>
                    {opt}
                  </div>
                );
              })}
            ...
            {isEx1Complete && (
              <button onClick={handleCheckExercise1} className="w-[80%] mt-4 bg-student-yellow text-outloud-blue font-black py-3.5 rounded-full shadow-lg active:scale-95 transition-transform uppercase tracking-widest text-sm animate-fade-in-up">
                CONTINUAR
              </button>
            )}
          </div>
      ...
      {(step === 'exercise2' || step === 'exercise3') && (
        <div className="relative z-10 w-full px-5 flex flex-col flex-grow animate-fade-in pb-8">
          <div className="mb-4 text-center shrink-0">
            <h2 className="text-[13px] text-outloud-blue font-montserrat font-black uppercase">
              LESSON 1: ACTIVITY {step === 'exercise2' ? '2' : '3'} - <span className="font-bold normal-case">{step === 'exercise2' ? 'Listen and repeat.' : 'Compare.'}</span>
            </h2>
            <p className="text-[11px] text-outloud-blue font-montserrat mt-1 px-2 leading-relaxed">
              {step === 'exercise2' ? (
                <>Press the <svg className="w-4 h-4 mx-0.5 inline" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.415 0A5.983 5.983 0 0115 10a5.984 5.984 0 01-1.757 4.243 1 1 0 01-1.415-1.415A3.984 3.984 0 0013 10a3.983 3.983 0 00-1.172-2.828 1 1 0 010-1.415z" clipRule="evenodd"></path></svg> icon to listen to the original audio... then, press the <svg className="w-4 h-4 mx-0.5 inline" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"></path></svg> icon to repeat and record your version.</>
              ) : (
          ...
          <div className="w-full max-w-sm mx-auto bg-[#2a3036] rounded-[2rem] border-4 border-[#3b434b] shadow-xl p-5 flex flex-col">
            <div className="flex flex-row items-center gap-4 mb-4">
              <div className="w-3/4 aspect-[4/5] rounded-xl overflow-hidden shadow-lg border border-gray-600 bg-black relative shrink-0">
                <img src="https://i.postimg.cc/CLmdVSQX/1(3).png" alt="Scene" className="w-full h-full object-cover" />
              </div>
              <div className="w-1/4 flex flex-col items-center justify-center space-y-6">
                <div className="flex flex-col items-center">
                  <span className="text-white text-[10px] font-bold font-montserrat mb-1">Listen</span>
                  <button onClick={playOriginalAudio} className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all ${activeAudio === 'original' ? 'bg-[#5b9bd5]' : 'bg-transparent hover:bg-white/10'} ${step === 'exercise2' && !hasRecorded ? 'ring-2 ring-green-500 ring-offset-4 ring-offset-[#2a3036]' : ''}`}>
                    <svg className="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.415 0A5.983 5.983 0 0115 10a5.984 5.984 0 01-1.757 4.243 1 1 0 01-1.415-1.415A3.984 3.984 0 0013 10a3.983 3.983 0 00-1.172-2.828 1 1 0 010-1.415z" clipRule="evenodd"></path></svg>
                  </button>
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-white text-[10px] font-bold font-montserrat mb-1">{step === 'exercise2' ? 'Record' : 'Compare'}</span>
                  {step === 'exercise2' ? (
                    <button onClick={toggleRecording} className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-transparent hover:bg-white/10'}`}>
                      <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"></path></svg>
                    </button>
                  ) : (
                    <button onClick={playRecordedAudio} className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all ${activeAudio === 'user' ? 'bg-[#5b9bd5]' : 'bg-transparent hover:bg-white/10'} ${step === 'exercise3' && !hasCompared ? 'ring-2 ring-green-500 ring-offset-4 ring-offset-[#2a3036]' : ''}`}>
                      <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
                    </button>
                  )}
                </div>
              </div>
            </div>
            <div className="text-center text-white font-montserrat text-sm font-medium tracking-wide mb-4">
            ...
            <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden mt-auto">
              <div className="h-full bg-student-yellow transition-all duration-100" style={{ width: `${audioProgress}%` }}></div>
            </div>
          </div>

          <div className="flex flex-col items-center justify-center mt-6 h-12">
            {step === 'exercise2' && hasRecorded && !isRecording && (
              <button onClick={handleContinueToEx3} className="w-[80%] max-w-[200px] bg-student-yellow text-outloud-blue font-black py-3 rounded-full shadow-lg active:scale-95 uppercase tracking-widest text-[11px] animate-fade-in-up">
                CONTINUAR
              </button>
            )}
            {step === 'exercise3' && hasCompared && (
              <div className="flex flex-row items-center justify-center space-x-3 w-full animate-fade-in-up">
                <button onClick={handleRetryEx2} className="w-12 h-12 flex items-center justify-center bg-white border-2 border-outloud-blue text-outloud-blue rounded-full shadow-md active:scale-95 shrink-0">
                  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 3.16L3 8" /><path d="M3 3v5h5" /></svg>
                </button>
                <button onClick={handleContinueToEx4} className="w-[60%] max-w-[200px] bg-student-yellow text-outloud-blue font-black py-3 rounded-full shadow-lg active:scale-95 uppercase tracking-widest text-[11px]">
                  CONTINUAR
                </button>
              </div>
            )}
      ...
      {/* STEP 7: EXERCISES 4 & 5 */}
      {step === 'exercises4_and_5' && (
        <div className="relative z-10 w-full px-5 flex flex-col flex-grow animate-fade-in pb-8">
          <div className="mb-8 w-full bg-white rounded-[2rem] p-5 shadow-lg">
            <h2 className="text-[13px] text-outloud-blue font-montserrat font-black uppercase mb-1">LESSON 1: ACTIVITY 4<br/><span className="font-bold normal-case">Grammar exercise.</span></h2>
            <p className="text-[10px] font-bold text-outloud-blue font-montserrat uppercase mb-5">FILL IN THE BLANK: <span className="font-normal normal-case">Complete the text.</span></p>
            <ul className="space-y-4 text-[13px] text-outloud-blue font-montserrat font-medium">
              <li className="flex items-center flex-wrap gap-y-2">
                <span className="font-bold mr-1">• Alan:</span> How much
                <input type="text" maxLength={6} value={act4Input1} onFocus={() => setAct4Touched1(true)} onChange={(e) => setAct4Input1(e.target.value)} className={`inline-block w-16 mx-2 border-b-2 bg-transparent text-center font-bold text-outloud-blue outline-none transition-all ${!act4Touched1 && act4Input1 === '' ? 'border-outloud-blue animate-pulse' : 'border-outloud-blue focus:border-student-yellow'}`} />
                a room?
              </li>
              <li className="flex items-center flex-wrap gap-y-2">
                <span className="font-bold mr-1">• Alan:</span> The WiFi
                <input type="text" maxLength={6} value={act4Input2} onFocus={() => setAct4Touched2(true)} onChange={(e) => setAct4Input2(e.target.value)} className={`inline-block w-16 mx-2 border-b-2 bg-transparent text-center font-bold text-outloud-blue outline-none transition-all ${!act4Touched2 && act4Input2 === '' ? 'border-outloud-blue animate-pulse' : 'border-outloud-blue focus:border-student-yellow'}`} />
                free.
              </li>
            </ul>
          </div>

          <div className="mb-6 w-full bg-white rounded-[2rem] p-5 shadow-lg">
            <h2 className="text-[13px] text-outloud-blue font-montserrat font-black uppercase mb-1">LESSON 1: ACTIVITY 5<br/><span className="font-bold normal-case">Vocabulary exercise.</span></h2>
            <p className="text-[10px] font-bold text-outloud-blue font-montserrat uppercase mb-5">CHOOSE THE RIGHT OPTION: <span className="font-normal normal-case">Click on the correct button.</span></p>
            <p className="text-sm font-bold text-outloud-blue font-montserrat mb-4">Dennis is a...</p>
            <div className="flex flex-col space-y-3 pl-2">
              {['Taxi driver', 'Recepcionist', 'Pilot'].map((opt) => (
                <div key={opt} onClick={() => setAct5Selection(opt)} className="flex items-center space-x-3 cursor-pointer">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${act5Selection === opt ? 'border-student-yellow' : 'border-outloud-blue/40'}`}>
                    {act5Selection === opt && <div className="w-2.5 h-2.5 bg-student-yellow rounded-full"></div>}
                  </div>
                  <span className="text-[13px] text-outloud-blue font-montserrat font-medium">{opt}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-auto flex justify-center">
            {isEx45Complete && (
              <button onClick={handleCheckExercises45} className="w-[80%] max-w-[250px] bg-student-yellow text-outloud-blue font-black py-3.5 rounded-full shadow-lg active:scale-95 uppercase tracking-widest text-[11px] animate-fade-in-up">
                CONTINUAR
              </button>
            )}
          </div>
      ...
      {step === 'results' && (
        <div className="relative z-10 w-full px-6 flex flex-col justify-center flex-grow animate-fade-in pb-12 pt-6">
          <div className="flex flex-col items-center text-center space-y-1 mb-8">
            <h1 className="text-[32px] font-black text-[#08203e] font-montserrat uppercase tracking-tight leading-none">A1: UNIT 1</h1>
            <h1 className="text-[32px] font-black text-[#08203e] font-montserrat uppercase tracking-tight leading-none">LESSON 1</h1>
            <h1 className="text-[32px] font-black text-[#08203e] font-montserrat uppercase tracking-tight leading-none mt-2">COMPLETED</h1>
          </div>
          <div className="w-full bg-white/80 backdrop-blur-sm rounded-[2rem] p-6 shadow-xl flex flex-col items-center mb-8 border border-white/50">
            <MobileProgressBar label="Listening:" percent={listeningScore} />
            <MobileProgressBar label="Reading:" percent={readingScore} />
            <MobileProgressBar label="Grammar:" percent={grammarScore} />
            <MobileProgressBar label="Comprehension:" percent={comprehensionScore} />
            <MobileProgressBar label="Speaking:" percent={speakingScore} />
            <span className="text-[5rem] font-black text-[#08203e] font-montserrat leading-none mt-4">{overallScore}%</span>
          </div>
          <button onClick={onReturnToRegister} className="w-full bg-student-yellow text-outloud-blue font-black py-4 rounded-2xl shadow-xl active:scale-95 flex flex-col items-center text-sm leading-tight animate-pulse tracking-wide">
...
const DesktopFreeLesson = (props) => {
  const {
    step, replayCount, isEnded, dragSlots, selectedPill, availableOptions,
    isRecording, hasRecorded, hasCompared, activeAudio, audioProgress,
    act4Input1, setAct4Input1, act4Input2, setAct4Input2, act4Touched1, setAct4Touched1,
    act4Touched2, setAct4Touched2, act5Selection, setAct5Selection,
    handleStart, handleContinueToVideo, handleContinueToEx1, getIframeUrl,
          ...
          <div className="flex flex-row items-center justify-center w-full max-w-6xl gap-4 md:gap-8 h-full">
            ...
            {step === 'video' && (
              <div className="flex flex-col items-center justify-center shrink-0">
                <button onClick={handleContinueToEx1} className={`w-14 h-14 md:w-16 md:h-16 rounded-full bg-outloud-blue hover:bg-blue-900 flex items-center justify-center text-white shadow-xl transition-all duration-300 ${isEnded ? 'animate-pulse ring-4 ring-student-yellow ring-offset-2 ring-offset-[#eef5fc] scale-110' : ''}`} title="Continuar">
      ...
      {step === 'exercise1' && (
        <div className="relative z-10 w-full max-w-[62rem] mx-auto px-6 md:px-16 lg:px-24 xl:px-32 flex flex-col flex-grow animate-fade-in pb-12">
          ...
          <div className="grid grid-cols-4 gap-2 md:gap-4 lg:gap-5 w-full shrink-0">
            {[{ id: 'slot1', img: 'https://i.postimg.cc/CLmdVSQX/1(3).png' }, { id: 'slot2', img: 'https://i.postimg.cc/Hx9d4tGZ/2(4).png' }, { id: 'slot3', img: 'https://i.postimg.cc/8P0DH5Y3/3(3).png' }, { id: 'slot4', img: 'https://i.postimg.cc/tTpHTV1S/4(3).png' }].map((col) => (
              <div key={col.id} className="flex flex-col items-center w-full space-y-2 md:space-y-3">
                ...
                <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDropOnSlot(e, col.id)} onClick={() => handleSlotClick(col.id)} className={`w-full h-8 md:h-10 lg:h-12 shrink-0 rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer overflow-hidden ${dragSlots[col.id] ? 'bg-transparent border-none' : 'bg-[#eef5fc]/50 border-2 border-dashed border-[#08203e]'} ${selectedPill ? 'ring-2 ring-student-yellow' : ''}`}>
                  {dragSlots[col.id] && (
                    <div draggable onDragStart={(e) => handleDragStart(e, dragSlots[col.id], col.id)} className="w-full h-full bg-[#08203e] text-white rounded-full flex items-center justify-center text-[8px] md:text-[10px] lg:text-[11px] font-bold font-montserrat text-center px-1 cursor-grab shadow-sm whitespace-pre-line leading-tight">
                      {dragSlots[col.id]}
                    </div>
                  )}
                </div>
          ...
          <div className="shrink-0 flex flex-col items-center w-full mt-4 md:mt-6">
            <div onDragOver={(e) => e.preventDefault()} onDrop={handleDropOnBank} className={`w-full grid grid-cols-4 gap-2 md:gap-4 lg:gap-5 ${isEx1Complete ? 'hidden' : ''}`}>
              {allOptions.map((opt) => {
                if (!availableOptions.includes(opt)) { return <div key={`empty-${opt}`} className="w-full h-8 md:h-10 lg:h-12"></div>; }
                return (
                  <div key={opt} draggable onClick={() => handlePillClick(opt, 'bank')} onDragStart={(e) => handleDragStart(e, opt, 'bank')} className={`w-full h-8 md:h-10 lg:h-12 rounded-full font-bold font-montserrat text-[8px] md:text-[10px] lg:text-[11px] text-center shadow-md cursor-grab hover:-translate-y-1 transition-all flex items-center justify-center whitespace-pre-line leading-tight select-none px-1 ${selectedPill === opt ? 'bg-student-yellow text-outloud-blue ring-2 ring-student-yellow/50 scale-105 z-10' : 'bg-[#08203e] text-white hover:bg-blue-900'}`}>
                    {opt}
                  </div>
                );
              })}
            ...
            {isEx1Complete && (
              <div className="w-full flex justify-center mt-2 animate-fade-in-up">
                <button onClick={handleCheckExercise1} className="bg-student-yellow text-outloud-blue font-black px-10 md:px-14 py-2.5 md:py-3 rounded-full shadow-lg transition-transform hover:scale-105 uppercase tracking-widest text-xs md:text-sm">CONTINUAR</button>
              </div>
            )}
            {isEx1Complete && (
            ...
            <div className="flex flex-row items-center justify-center gap-6 md:gap-12 w-full flex-grow min-h-0">
              ...
              <div className="flex flex-col justify-center items-center space-y-8">
                ...
                <div className="flex flex-col items-center justify-center min-h-[4rem] pt-2">
                  {step === 'exercise2' && hasRecorded && !isRecording && (
                    <button onClick={handleContinueToEx3} className="bg-student-yellow text-outloud-blue font-black px-6 md:px-8 py-2 md:py-3 rounded-full shadow-xl transition-transform hover:scale-105 animate-pulse uppercase tracking-widest text-[10px] md:text-xs">CONTINUAR</button>
                  )}
                  {step === 'exercise3' && hasCompared && (
                    <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3 animate-fade-in-up">
                      <button onClick={handleRetryEx2} title="Retry Exercise" className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white border-2 border-outloud-blue text-outloud-blue hover:bg-blue-50 rounded-full shadow-md transition-transform hover:scale-105 shrink-0">
                        <svg className="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 3.16L3 8" /><path d="M3 3v5h5" /></svg>
                      </button>
                      <button onClick={handleContinueToEx4} className="bg-student-yellow text-outloud-blue font-black px-6 md:px-8 py-2 md:py-3 rounded-full shadow-xl transition-transform hover:scale-105 animate-pulse uppercase tracking-widest text-[10px] md:text-xs">CONTINUAR</button>
                    </div>
                  )}
                </div>
              </div>
      ...
      {step === 'exercises4_and_5' && (
        <div className="relative z-10 w-full max-w-[50rem] mx-auto px-6 md:px-12 flex flex-col flex-grow animate-fade-in pb-12 pt-4">
          <div className="mb-10 w-full">
            <h2 className="text-sm md:text-lg lg:text-xl text-outloud-blue font-montserrat"><span className="font-black uppercase">LESSON 1: ACTIVITY 4</span> - <span className="font-bold">Grammar exercise.</span></h2>
            <p className="text-[10px] md:text-xs lg:text-sm font-bold text-outloud-blue font-montserrat uppercase tracking-wide mt-2 mb-6">FILL IN THE BLANK: <span className="font-normal">Complete the text with the correct word from the lesson.</span></p>
            <ul className="space-y-6 text-sm md:text-lg text-outloud-blue font-montserrat font-medium ml-2 md:ml-4">
              <li className="flex items-center flex-wrap gap-y-2">
                <span className="font-bold mr-2">• Alan:</span> How much
                <input type="text" maxLength={6} value={act4Input1} onFocus={() => setAct4Touched1(true)} onChange={(e) => setAct4Input1(e.target.value)} className={`inline-block w-20 md:w-24 mx-2 md:mx-3 border-b-2 bg-transparent text-center font-bold text-outloud-blue outline-none transition-all ${!act4Touched1 && act4Input1 === '' ? 'border-outloud-blue animate-pulse' : 'border-outloud-blue focus:border-student-yellow'}`} />
                a room?
              </li>
              <li className="flex items-center flex-wrap gap-y-2">
                <span className="font-bold mr-2">• Alan:</span> The WiFi
                <input type="text" maxLength={6} value={act4Input2} onFocus={() => setAct4Touched2(true)} onChange={(e) => setAct4Input2(e.target.value)} className={`inline-block w-20 md:w-24 mx-2 md:mx-3 border-b-2 bg-transparent text-center font-bold text-outloud-blue outline-none transition-all ${!act4Touched2 && act4Input2 === '' ? 'border-outloud-blue animate-pulse' : 'border-outloud-blue focus:border-student-yellow'}`} />
                free.
              </li>
            </ul>
          </div>
          <div className="mb-10 w-full mt-4 md:mt-8">
            <h2 className="text-sm md:text-lg lg:text-xl text-outloud-blue font-montserrat"><span className="font-black uppercase">LESSON 1: ACTIVITY 5</span> - <span className="font-bold">Vocabulary exercise.</span></h2>
            <p className="text-[10px] md:text-xs lg:text-sm font-bold text-outloud-blue font-montserrat uppercase tracking-wide mt-2 mb-6">CHOOSE THE RIGHT OPTION: <span className="font-normal">Click on the correct button.</span></p>
            <p className="text-base md:text-xl font-bold text-outloud-blue font-montserrat mb-6 ml-2 md:ml-4">Dennis is a...</p>
            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-8 md:space-x-12 ml-2 md:ml-4 space-y-4 sm:space-y-0">
              {['Taxi driver', 'Recepcionist', 'Pilot'].map((opt) => (
                <div key={opt} onClick={() => setAct5Selection(opt)} className="flex items-center space-x-3 cursor-pointer group">
                  <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full border-2 flex items-center justify-center transition-colors ${act5Selection === opt ? 'border-student-yellow' : 'border-outloud-blue/40 group-hover:border-outloud-blue'}`}>
          ...
          <div className="mt-auto h-16 flex items-center justify-center">
            {isEx45Complete && (
              <button onClick={handleCheckExercises45} className="bg-student-yellow text-outloud-blue font-black px-12 md:px-16 py-3 rounded-full shadow-lg transition-transform hover:scale-105 animate-pulse uppercase tracking-widest text-sm md:text-base animate-fade-in-up">CONTINUAR</button>
            )}
          </div>
        </div>
      )}
...
const FreeLesson = (props) => {
  ...
  const handleContinueToVideo = async () => {
    ...
    try {
      const { data: supabaseData, error: supabaseError } = await supabase.from('free_lessons').insert([{}]).select('id').single();
  ...
  const handleRetryEx2 = () => {
    setMetrics(prev => ({ ...prev, audioRetries: prev.audioRetries + 1 }));
  ...
  const sharedProps = {
    step, setStep, replayCount, isEnded, metrics, setMetrics,
    selectedPill, setSelectedPill, dragSlots, setDragSlots, availableOptions, setAvailableOptions,
    isRecording, hasRecorded, hasCompared, hasListenedOriginal, activeAudio, audioProgress,
    act4Input1, setAct4Input1, act4Input2, setAct4Input2, act4Touched1, setAct4Touched1, act4Touched2, setAct4Touched2, act5Selection, setAct5Selection,
    handleStart, handleContinueToVideo, handleContinueToEx1, getIframeUrl, handleDragStart, handleDropOnSlot, handleDropOnBank, handlePillClick, handleSlotClick, processMove, handleCheckExercise1, isEx1Complete, playOriginalAudio, playRecordedAudio, toggleRecording, handleContinueToEx3, handleRetryEx2, handleContinueToEx4, isEx45Complete, handleCheckExercises45, listeningScore, comprehensionScore, grammarScore, readingScore, speakingScore, overallScore, allOptions,
    ...props
  };
