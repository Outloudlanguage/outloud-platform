...
const FinancesPage = () => {
  ...
  if (impersonatingStudent) {
    return (
      <>
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] bg-red-600 border-2 border-white/50 backdrop-blur-md text-white text-center p-1 pl-4 rounded-full font-black tracking-widest text-[10px] md:text-xs flex items-center gap-4 shadow-[0_10px_40px_rgba(220,38,38,0.6)] animate-slide-down">
          <span className="truncate max-w-[150px] md:max-w-xs drop-shadow-md">VIEWING: {impersonatingStudent.first_name}</span>
          <button onClick={() => setImpersonatingStudent(null)} className="bg-black/50 hover:bg-black text-white px-5 py-2.5 rounded-full transition-colors shrink-0 shadow-inner flex items-center gap-2">
